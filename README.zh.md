...
`intervalMinutes` 是在定时器上弦时读的，而改它那次保存会**重新上弦**——所以把 30 改成
5，设置页说生效就生效。0.4.3 之前不是这样：`armDigest` 在定时器已经跑着的时候直接返回，
所以对一个已经挂着事件的 relay，那次保存是空转的，待发的摘要仍按**第一条事件到达时**的
间隔发。值其实存对了，这才最难发现。

共用同一个 tick 的 outbox 心跳也一样，而且在那边更糟：它用**首次上弦时捕获的那个
delay** 重新上弦，于是心跳**永远**停在旧间隔上，每次刷新都是。

如果要改这块，有一个坑值得知道：`disarmDigest()` 会把挂起队列一起清空。合批被关掉时
这是对的，仅仅改间隔时则是灾难——每一条攒着的通知都会在"本该重新排定它们投递时间"的那次
保存上被丢掉。改期走的是 `rearmDigest()`，只丢定时器不动队列；另有一个坏构建变体把 naive
写法换回来，证明门禁还抓得住。
